...
# --- clean-up
gss <- gss |>
  mutate(abany = as.character(abany)) |>
  mutate(abany = case_when(abany == "Yes" ~ 1,
                           abany == "No" ~ 0,
                           .default = NA_real_)) |>
  drop_na() |>
  mutate(n = n(), .by = pid) |> 
...
d <- gridsearch::gridSearch(
  ...
  data = gss,
  ## vars
  yname = 'abany', tname = 'wave', pname = 'pid',
  ## patterns
  pattern = "contingency", steps = 30,
  ## reliability
  reliability = 0.86, ## this is from Hout and Hasting (2016)
  workers = 10, seed = 112358)

saveRDS(d, "./data/output/abany.rds")
